...
def delete_message(peer_id: int, msg: dict):
    """Удаляет сообщение. Пробует разные поля ID."""
...
def process_message(data: dict):
    try:
        # Перезагружаем данные из БД при каждом сообщении
        storage.data = db_get()
        ...
        # Проверяем активна ли беседа (только для беседы, не для ЛС)
        if peer_id > 2000000000 and not storage.is_chat_active(peer_id):
            # Беседа не активирована — только команда /start
        ...
        # Тишина — удаляем сообщения не-модераторов
        if peer_id > 2000000000 and storage.is_silence(peer_id):
            priority = get_user_priority(user_id)
            if priority == 0:  # не модератор
                delete_message(peer_id, msg)
                return
